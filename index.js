...
<body>
  ...
  <script type="module">
    ...
    function navButtons(){return orderedTabs().map(tab=>'<button class="nav-button '+(state.tab===tab.id?'active':'')+'" data-tab="'+escapeHtml(tab.id)+'"><span class="nav-dot" style="background:'+escapeHtml(tab.color||'#ff6a1a')+'"></span>'+escapeHtml(tab.name)+'<span class="count">'+tabCount(tab.id)+'</span></button>').join('')+'<button class="nav-button '+(state.tab==='reminders'?'active':'')+'" data-tab="reminders"><span class="nav-dot reminder"></span>Reminders<span class="count">'+tabCount('reminders')+'</span></button>'}
    function bottomNavHtml(){return orderedTabs().map(tab=>'<button class="'+(state.tab===tab.id?'active':'')+'" data-tab="'+escapeHtml(tab.id)+'">'+escapeHtml(tab.name)+'</button>').join('')+'<button class="'+(state.tab==='reminders'?'active':'')+'" data-tab="reminders">Reminders</button><button data-action="manage-tabs">+ Tab</button>'}
    ...
    function bindEvents(){
      ...
      $$('[data-action="new"]').forEach(b=>b.onclick=()=>createEntry(state.tab==='reminders'?(state.tabs[0]?.id||'work'):state.tab));
    ...
    function showTabManager(){showModal('<h2>Journal tabs</h2><p class="modal-copy">Create spaces for projects, areas of life, or anything you want to keep separate. Reminders always stays available.</p><div class="tab-manager">'+orderedTabs().map(tab=>'<div class="tab-manager-row"><span class="color-dot" style="background:'+escapeHtml(tab.color||'#ff6a1a')+'"></span><span>'+escapeHtml(tab.name)+'</span><button class="button danger" data-delete-tab="'+escapeHtml(tab.id)+'">Delete</button></div>').join('')+'<div class="tab-manager-row"><span class="color-dot" style="background:#92b39a"></span><span>Reminders</span><small>Permanent</small></div></div><div class="modal-actions"><button class="button" data-close>Close</button><button class="button primary" data-add-tab>+ Add tab</button></div>');$('[data-close]').onclick=closeModal;$('[data-add-tab]').onclick=showAddTabModal;$$('[data-delete-tab]').forEach(button=>button.onclick=()=>showDeleteTabModal(button.dataset.deleteTab))}
    function showAddTabModal(){showModal('<h2>Add a journal tab</h2><form id="tabForm"><div class="field"><label for="tabName">Tab name</label><input id="tabName" required maxlength="32" placeholder="Ideas, Family, Learning…"></div><div class="field"><label for="tabColor">Color</label><input id="tabColor" type="color" value="#ff6a1a"></div><div class="modal-actions"><button class="button" type="button" data-close>Cancel</button><button class="button primary">Add tab</button></div></form>');$('[data-close]').onclick=closeModal;$('#tabName').focus();$('#tabForm').onsubmit=async event=>{event.preventDefault();const name=$('#tabName').value.trim();if(!name)return;const tab={id:'tab-'+uid(),user_id:state.session?.user?.id||'local-user',name,color:$('#tabColor').value,position:orderedTabs().length,created_at:nowIso()};try{await data.saveTab(tab);state.tab=tab.id;state.selectedId=null;closeModal();render();toast(name+' tab added')}catch(error){toast(error.message||'Could not add tab','error')}}}
    ...
    async function showSystemReminder(r){if(!('Notification' in window)||Notification.permission!=='granted')return;const options={body:r.title,tag:'reminder-'+r.id,renotify:true,requireInteraction:true,icon:'/icon.svg',badge:'/icon.svg',data:{url:'/'}};try{const registration=await navigator.serviceWorker?.ready;if(registration)await registration.showNotification(CONFIG.appName,options);else new Notification(CONFIG.appName,options)}catch{try{new Notification(CONFIG.appName,options)}catch{}}}
...
const serviceWorker = `const CACHE='daybook-v3';self.addEventListener('install',e=>{self.skipWaiting();e.waitUntil(caches.open(CACHE).then(c=>c.addAll(['/','/manifest.webmanifest','/icon.svg'])))});self.addEventListener('activate',e=>e.waitUntil(Promise.all([caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE).map(k=>caches.delete(k)))),self.clients.claim()])));self.addEventListener('fetch',e=>{if(e.request.method==='GET'&&new URL(e.request.url).origin===self.location.origin)e.respondWith(fetch(e.request).then(r=>{const copy=r.clone();caches.open(CACHE).then(c=>c.put(e.request,copy));return r}).catch(()=>caches.match(e.request).then(r=>r||caches.match('/'))))});self.addEventListener('notificationclick',e=>{e.notification.close();e.waitUntil(clients.matchAll({type:'window',includeUncontrolled:true}).then(list=>{for(const client of list){if('focus' in client){client.navigate('/');return client.focus()}}return clients.openWindow('/')}))});`;
