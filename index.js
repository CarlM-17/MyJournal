...
<body>
  ...
  <script type="module">
    ...
    const state = {
      session: null, entries: [], reminders: [], attachments: [], tabs: [], selectedId: null,
    ...
    async function migrateLocalJournalIfNeeded() {
      ...
      if (!localEntries.length && !localReminders.length && !localTabs.length) return;
    ...
    const data = {
      async load() {
        if (state.localMode) {
          state.entries = localRead('entries'); state.reminders = localRead('reminders'); state.attachments = localRead('attachments'); state.tabs = localRead('tabs', DEFAULT_TABS.map(tab=>({...tab}))); if(!state.tabs.length)state.tabs=DEFAULT_TABS.map(tab=>({...tab})); localWrite('tabs',state.tabs); recoverDrafts(); return;
        }
        const [entries, reminders, attachments, tabs] = await Promise.all([
        ...
        const error = entries.error || reminders.error || attachments.error || tabs.error;
        if (error) throw error;
        state.entries = entries.data; state.reminders = reminders.data; state.attachments = attachments.data; state.tabs = tabs.data;
      ...
      async deleteTab(tab) {
        ...
        if(state.localMode){state.tabs=state.tabs.filter(item=>item.id!==tab.id);state.entries=state.entries.filter(entry=>entry.space!==tab.id);state.reminders=state.reminders.filter(reminder=>reminder.space!==tab.id);state.attachments=state.attachments.filter(item=>!entryIds.includes(item.entry_id));localWrite('tabs',state.tabs);localWrite('entries',state.entries);localWrite('reminders',state.reminders);localWrite('attachments',state.attachments);return;}
        ...
        state.tabs=state.tabs.filter(item=>item.id!==tab.id);state.entries=state.entries.filter(entry=>entry.space!==tab.id);state.reminders=state.reminders.filter(reminder=>reminder.space!==tab.id);state.attachments=state.attachments.filter(item=>!entryIds.includes(item.entry_id));
      }
    };
    ...
    function bindEvents(){
      const crumb=$('.crumb');if(crumb&&selectedEntry())crumb.textContent=tabName(selectedEntry().space)+' note';
      ...
      $$('[data-tab]').forEach(b=>b.onclick=async()=>{if(state.listening)await stopVoice();state.tab=b.dataset.tab;state.search='';state.filter='all';state.mobileEditorOpen=false;if(state.tab!=='reminders'){const first=state.entries.find(e=>e.space===state.tab&&!e.archived);state.selectedId=first?.id||null;}render();});
    ...
    async function refreshCloudData() {
      ...
      try{
        ...
        const error=entries.error||reminders.error||attachments.error||tabs.error;if(error)throw error;
        ...
        state.reminders=reminders.data;state.attachments=attachments.data;state.tabs=tabs.data.length?tabs.data:state.tabs;if(state.tab!=='reminders'&&!state.tabs.some(tab=>tab.id===state.tab)){state.tab=state.tabs[0]?.id||'work';state.selectedId=state.entries.find(entry=>entry.space===state.tab&&!entry.archived)?.id||null}await hydrateAttachmentUrls();
    ...
    async function signOut(){if(state.listening)stopVoice();stopAlarmSound();clearInterval(state.syncTimer);clearInterval(state.dueTimer);if(cloudAvailable)await supabase.auth.signOut();state.session=null;state.entries=[];state.tabs=[];renderAuth()}
    async function startApp(){try{$('#loading').classList.remove('hidden');await data.load();state.tab=state.tabs.some(tab=>tab.id===state.tab)?state.tab:(state.tabs[0]?.id||'work');const first=state.entries.find(e=>e.space===state.tab&&!e.archived);state.selectedId=first?.id||null;render();checkDueReminders();clearInterval(state.dueTimer);state.dueTimer=setInterval(checkDueReminders,15000);setupCloudRefresh()}catch(error){$('#loading').classList.add('hidden');toast(error.message||'Could not load your journal','error');renderAuth()}}
