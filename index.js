...
<body>
  ...
  <script type="module">
    ...
    const state = {
      ...
      voicePendingInterim:'', voiceStream:null, voiceRestartTimer:null, voiceSaveTimer:null, voiceIdleRestarts:0,
    ...
    async function toggleVoice(target='content'){
      ...
      state.voiceTarget=target;state.voiceShouldContinue=true;state.listening=true;state.voiceRecentFinals=new Map();state.voicePendingInterim='';state.voiceIdleRestarts=0;
      if(isMobileVoice()&&navigator.mediaDevices?.getUserMedia){try{state.voiceStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}})}catch(error){if(error.name==='NotAllowedError'){state.voiceShouldContinue=false;state.listening=false;toast('Microphone permission is required','error');return}}}
    ...
    function startVoiceCycle(){
      ...
      recognition.onend=async()=>{if(state.recognition===recognition)state.recognition=null;if(state.voicePendingInterim)commitSpeechChunk(state.voicePendingInterim,recognition);await flushVoiceSave();if(state.voiceShouldContinue){if(!heardSpeech)state.voiceIdleRestarts++;const delay=heardSpeech?180:Math.min(1800,450+state.voiceIdleRestarts*250);const save=$('#saveState');if(save)save.textContent=heardSpeech?'Listening…':'Microphone ready…';clearTimeout(state.voiceRestartTimer);state.voiceRestartTimer=setTimeout(startVoiceCycle,delay)}else{releaseVoiceStream();state.listening=false;render()}};
